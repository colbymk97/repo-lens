...
async function main(): Promise<void> {
  ...
  if (!config) {
    throw new Error('Missing OPENAI_API_KEY for search task eval.');
  }
  ...
  try {
    ...
    const summary = await runSearchTaskEvaluation(retriever, config, dataset);
    console.log(
      `Search task eval complete: ${summary.dataset.taskCount} tasks, ` +
      `success rate ${summary.metrics.successRate.toFixed(3)}`,
    );
  } finally {
    db.close();
  }
